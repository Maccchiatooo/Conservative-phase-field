...
using buffer_ft = Kokkos::View<double ****,  Kokkos::CudaUVMSpace>;
...
struct LBM
{
        ...
        : comm(comm_), glx(sx), gly(sy), glz(sz), tau0(tau0), tau1(tau1), taum(taum), rho0(rho0),rho1(rho1), r_x(r_x), r_y(r_y), r_z(r_z),sigma(sigma)
    {
        ...
        int f_edgestart_send[12][4] = {{0, ghost, 0, lz - 2 * ghost}, {0, lx - 2 * ghost, 0, lz - 2 * ghost}, {0, ghost, 0, ghost}, {0, lx - 2 * ghost, 0, ghost}, {0, 0, ghost, lz - 2 * ghost}, {0, 0, ghost, ghost}, {0, 0, ly - 2 * ghost, lz - 2 * ghost}, {0, 0, ly - 2 * ghost, ghost}, {0, lx - 2 * ghost, ghost, 0}, {0, ghost, ghost, 0}, {0, lx - 2 * ghost, ly - 2 * ghost, 0}, {0, ghost, ly - 2 * ghost, 0}};
        int f_edgestart_recv[12][4] = {{0, lx - ghost, 0, 0}, {0, 0, 0, 0}, {0, lx - ghost, 0, lz - ghost}, {0, 0, 0, lz - ghost}, {0, 0, ly - ghost, 0}, {0, 0, ly - ghost, lz - ghost}, {0, 0, 0, 0}, {0, 0, 0, lz - ghost}, {0, 0, ly - ghost, 0}, {0, lx - ghost, ly - ghost, 0}, {0, 0, 0, 0}, {0, lx - ghost, 0, 0}};

        int f_pointstart_send[8][4] = {{0, lx - 2 * ghost, ly - 2 * ghost, ghost}, {0, lx - 2 * ghost, ghost, ghost}, {0, lx - 2 * ghost, ghost, lz - 2 * ghost}, {0, lx - 2 * ghost, ly - 2 * ghost, lz - 2 * ghost}, {0, ghost, ghost, lz - 2 * ghost}, {0, ghost, ly - 2 * ghost, ghost}, {0, ghost, ghost, ghost}, {0, ghost, ly - 2 * ghost, lz - 2 * ghost}};
        int f_pointstart_recv[8][4] = {{0, 0, 0, lz - ghost}, {0, 0, ly - ghost, lz - ghost}, {0, 0, ly - ghost, 0}, {0, 0, 0, 0}, {0, lx - ghost, ly - ghost, 0}, {0, lx - ghost, 0, lz - ghost}, {0, lx - ghost, ly - ghost, lz - ghost}, {0, lx - ghost, 0, 0}};
        ...
        for (int i = 0; i < 6;i++){
            ...
            p2= f_facestart_send;
            p3= f_facestart_recv;
            MPI_Type_create_subarray(4, f_array, *(p1 + i / 2), *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &f_face_send[i]);
            MPI_Type_commit(&f_face_send[i]);
            MPI_Type_create_subarray(4, f_array, *(p1 + i / 2), *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &f_face_recv[i]);
        ...
        for (int i = 0; i < 12;i++){
            ...
            p2= f_edgestart_send;
            p3= f_edgestart_recv;
            MPI_Type_create_subarray(4, f_array, *(p1 + i / 4), *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &f_edge_send[i]);
            MPI_Type_commit(&f_edge_send[i]);
            MPI_Type_create_subarray(4, f_array, *(p1 + i / 4), *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &f_edge_recv[i]);
        ...
        for (int i = 0; i < 8;i++){
            ...
            MPI_Type_create_subarray(4, f_array, f_point_len, *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &f_point_send[i]);
            MPI_Type_commit(&f_point_send[i]);
            MPI_Type_create_subarray(4, f_array, f_point_len, *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &f_point_recv[i]);
        ...
        for (int i = 0; i < 6;i++){
            ...
            p2 = u_facestart_send;
            p3 = u_facestart_recv;
            MPI_Type_create_subarray(3, u_array, *(p1 + i / 2), *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &u_face_send[i]);
            MPI_Type_commit(&u_face_send[i]);
            MPI_Type_create_subarray(3, u_array, *(p1 + i / 2), *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &u_face_recv[i]);
        ...
        for (int i = 0; i < 12;i++){
            ...
            p2= u_edgestart_send;
            p3= u_edgestart_recv;
            MPI_Type_create_subarray(3, u_array, *(p1 + i / 4), *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &u_edge_send[i]);
            MPI_Type_commit(&u_edge_send[i]);
            MPI_Type_create_subarray(3, u_array, *(p1 + i / 4), *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &u_edge_recv[i]);
        ...
        for (int i = 0; i < 8;i++){
            ...
            MPI_Type_create_subarray(3, u_array, u_point_len, *(p2 + i), MPI_ORDER_C, MPI_DOUBLE, &u_point_send[i]);
            MPI_Type_commit(&u_point_send[i]);
            MPI_Type_create_subarray(3, u_array, u_point_len, *(p3 + i), MPI_ORDER_C, MPI_DOUBLE, &u_point_recv[i]);
    ...
    void exchange_u(Kokkos::View<double ***, Kokkos::CudaUVMSpace> u);
